...
// Helper: map Department document to frontend shape
...
const createDepartment = async (req, res) => {
  try {
    ...
    // Re-fetch with populated head for consistent response
    const saved = await Department.findById(department._id).populate({ 
      path: 'head', 
      select: 'name profileImage' 
    });
...
const updateDepartment = async (req, res) => {
  try {
    ...
    // Populate head for response
    department = await Department.findById(department._id).populate({ 
      path: 'head', 
      select: 'name profileImage' 
    });
...
const searchDepartments = async (req, res) => {
  try {
    ...
    const filtered = (departments || []).filter((d) => 
      regex.test(d.name) || 
      regex.test(d.description || "") || 
      regex.test(d.head?.name || "")
    );
...
const getDepartmentsPublicList = async (req, res) => {
  try {
